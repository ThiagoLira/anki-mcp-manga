...
`CardAgent.__init__` probes `local_llm_url` (default `http://100.86.254.13:9090/v1` — jobim tailscale IP, port 9090 — see `src/config.py`) at startup and uses it if reachable; otherwise falls back to OpenRouter (`OPENROUTER_API_KEY`). The local backend is the `llama-server-mtp-cuda` pod in the sibling `homelab` repo's `k8s/llama-server-mtp-cuda.yaml` — Qwen3.6-27B UD-Q4_K_XL with MTP speculative decoding on jobim's 3090 Ti, ~2.3× the throughput of the Vulkan-on-Strix-Halo janus pod it replaced. Port 9090, not 8080, because port 8080 on every node IP is claimed cluster-wide by the anki-sync-external LoadBalancer Service; 9080 is the older non-MTP janus llama-server, still running as a fallback target if needed.
